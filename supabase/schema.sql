...
CREATE TYPE position_action_type AS ENUM ('raise_stop', 'move_stop_to_breakeven', 'take_partial', 'reduce_position', 'close_full', 'hold');
CREATE TYPE action_execution_state AS ENUM ('pending', 'acknowledged', 'executed');
...
CREATE TABLE signals (
    ...
    execution_type VARCHAR(20) DEFAULT 'stock',
    execution_symbol VARCHAR(40),
    execution_isin VARCHAR(20),
    issuer VARCHAR(60),
    target_leverage DOUBLE PRECISION,
    knockout_level DOUBLE PRECISION,
    instrument_currency VARCHAR(10),
...
-- ─── POSITION ACTIONS (management instructions from bot) ────────────────────
